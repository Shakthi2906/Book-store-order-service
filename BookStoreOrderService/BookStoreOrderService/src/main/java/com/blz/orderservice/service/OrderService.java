...
public class OrderService implements IOrderService{
	...
	public OrderBookModel placeOrder(Long cartId, Long addressId, String token) {
		...
		if (isUserPresent.getErrorCode() == 200) {
			CartResponse isCartPresent = restTemplate.getForObject("http://BOOKSTORE-CART-SERVICE:8051/cartservice/getcart/" + cartId, CartResponse.class);
			if (isUserPresent.getErrorCode() == 200) {
				if (isUserPresent.getObject().getUserId() == isCartPresent.getObject().getUserId()) {
					...
					CartResponse isPresent = restTemplate.getForObject("http://BOOKSTORE-CART-SERVICE:8051/cartservice/delete/" + order.getCartId(), CartResponse.class);
					return order;
				}
	...
	public OrderBookModel cancelOrder(Long orderId, String token) {
		...
		if (isUserPresent.getErrorCode() == 200) {
			...
			if (isOrderPresent.isPresent()) {
				...
				BookResponse isBookIdPresent = restTemplate.getForObject("http://BOOKSTORE-BOOK-SERVICE:8050/bookdetails/updatequantity/" + 
